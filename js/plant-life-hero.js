/* GrowAi hero — Plant Life (Verlet) — tools + verlet + plant, orden correcto */
...
var canvas = null;
var ctx = null;
var canvRatio = 1;  // canvas ratio, as canvas size to lowest of window width or height
...
function updateSpans(currentIteration) {
  for (var i = 0; i < spans.length; i++) {
    var thisSpanIterations = Math.round(rigidity * spanStrength(spans[i]));
...
////---(plants at boot: growAiPlantLifeBoot)---////
...
function Plant( xLocation ) {
  ...
  this.isAlive = true;
  //settings
  this.forwardGrowthRate = gravity * Tl.rfb(35,50);  // (rate of cross spans increase per frame)
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", growAiPlantLifeBoot);
} else {
  growAiPlantLifeBoot();
}
